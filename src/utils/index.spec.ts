...
describe("pad()", () => {
    it("gives 0012 for 12", () => expect(pad(12, 4)).toEqual("0012"))
...
describe("toDictonary()", () => {
    ...
    it("gets empty dictonary if no getKey param is given", () => expect(toDictonary(["foo"], null as any)).toEqual({}))
    ...
    it("gets maped array of objects with duplicated value using passed getValue param", () =>
        expect(toDictonary([{ id: "foo" }, { id: "bar" }, { id: "foo", value: 2 }], s => s.id, s => s.value)).toEqual({
            foo: 2,
            bar: undefined
        }))
})
...
describe("remap()", () => {
    it("remaps", () => {
        ...
        expect(remap({ foo: 1, bar: 2 }, (_, key, index) => key.toUpperCase() + index.toString(), v => v + 1)).toEqual({
            FOO0: 2,
            BAR1: 3
        })
    })
